...
int main()
{
	...
	//Let's start in 1D; using the "Split-Step" algorithm.
...
//in the future, replace this with a file read to keep from having to recompile to change parameters
	//physical parameters
	double length = 1; // height in meters
	double mass = mass_of_potassium39_kg; //mass of atom in kg
	double trap_frequency = 50; //frequency of harmonic trap, in Hertz
	double N = 10000; //number of atoms in trap
	double time = 10; //length of simulation in seconds
	double characteristic_size = 0.000001; //characteristic length of the condensate, in meters
	...
	int nx = 64; //number of space steps in x-axis, must be a power of 2;
	int nt = 1000; //number of time steps

	double x_stepsize = length / nx; //physical size of single step in x-axis, in meters
...
/*		ofstream output_file;
	...
	for (int x = 0; x < nx; x++) {
		Psi[x] = exp(-pow(x, 2) / nx);
	}

	D_1::WritePsi_Abs_SquaredToFile(Psi, nx, "C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\step_initial.csv");
	...
	for (int t = 0; t < nt; t++) {
		...
		for (int x = 0; x < nx; x++) {
			Psi[x] = Psi[x] *
				((Complex(0, -0.5 * t_stepsize / epsilon * (pow(x*x_stepsize, 2) / 2 + K * (Psi[x].Abs_squared())))).Exp_pure_imaginary());
		}

	//	D_1::WritePsi_Abs_SquaredToFile(Psi, nx, "C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\Abs\\step_" + to_string(t) + "0.csv");
	//	D_1::WritePsi_ArgToFile(Psi, nx, "C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\Arg\\step_" + to_string(t) + "0.csv");
		//Solve for  Psi**_j as a function of Psi*_j
		FFT_naive(Psi, nx);

		for (int k = 0; k < nx; k++) {
			Psi[k] = Psi[k] *
				(Complex(0, -0.5*epsilon*t_stepsize*(pow(2.0*pi*k /length, 2) )).Exp_pure_imaginary());
//cout << (Complex(0, -0.5*epsilon*t_stepsize*(pow(2.0*pi*k / length, 2))).Exp_pure_imaginary());
		}

		IFT_1D(Psi, nx);
//D_1::WritePsi_Abs_SquaredToFile(Psi, nx, "C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\Abs\\step_" + to_string(t) + "1.csv");
//D_1::WritePsi_ArgToFile(Psi, nx, "C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\Arg\\step_" + to_string(t) + "1.csv");
		...
		D_1::WritePsi_Abs_SquaredToFile(Psi, nx, "C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\Abs\\step_" + to_string(t) + ".csv");
		D_1::WritePsi_ArgToFile(Psi, nx, "C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\Arg\\step_" + to_string(t) + ".csv");

	}

	//Check for conserved quantities and write summary file
	std::ofstream summary_file;
	summary_file.open("C:\\Users\\Grant\\Desktop\\CPPGPE\\TestOutput\\Summary.txt");

	summary_file << "CPP_GPE1D\n";
	summary_file << "length = " << length << '\n'; 
	summary_file << "mass = " << mass << '\n';
	summary_file << "trap_frequency = " << trap_frequency << '\n'; 
	summary_file << "N = " << N << '\n';
	summary_file << "time = " << time << '\n';
	summary_file << "characteristic size = " << characteristic_size << '\n';
	summary_file << "epsilon = " << epsilon << '\n';
	summary_file << "scattering length = " << a << '\n';
	summary_file << "harmonic trap lengthscale =" << a0 << '\n';
	summary_file << "healing length = " << healing_length << '\n';
	summary_file << "Interaction parameter K = " << K << '\n';


	summary_file << "nx = " << nx << '\n';
	summary_file << "nt = " << nt << '\n'; 

	summary_file << "x_stepsize = " << x_stepsize << '\n'; 
	summary_file << "t_stepsize = " << t_stepsize << '\n'; 
